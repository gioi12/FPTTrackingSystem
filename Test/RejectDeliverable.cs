...
{
    [TestClass]
    public class RejectDeliverable
    {
        ...
        [TestMethod]
        public async System.Threading.Tasks.Task RejectDeliverable_WithPendingStatus_ShouldReturnRejected()
        {
            // Arrange
            var userId = 1;
            var groupId = 1;
            var deliverableId = 1;
            var note = "Needs improvement";
            var semesterId = 1;
            ...
            // Act
            var result = await _deliverableService.RejectDeliverable(groupId, deliverableId, note);

            // Assert
            result.Should().Be(ProgressEnum.Rejected);
            ...
            _mockDeliverableRepository.Verify(x => x.UpdateDeliverable(
                It.Is<Deliverable>(d =>
                    d.DeliverableGroups.Any(dg =>
                        ...
                        dg.Status == ProgressEnum.Rejected &&
                        dg.Note == note))),
                Times.Once);
        }

        [TestMethod]
        public async System.Threading.Tasks.Task RejectDeliverable_UnsubmittedStatus_ShouldThrowException()
        {
            // Arrange
            var userId = 1;
            var groupId = 1;
            ...
            Func<System.Threading.Tasks.Task> act = async () => await _deliverableService.RejectDeliverable(groupId, deliverableId, null);
        ...
        [TestMethod]
        public async System.Threading.Tasks.Task RejectDeliverable_GroupNotFound_ShouldThrowException()
        {
            // Arrange
            var groupId = 1;
            var deliverableId = 1;
            var reason = "Invalid submission";
    ...
    await _deliverableService.RejectDeliverable(groupId, deliverableId, reason);
        ...
        [TestMethod]
        public async System.Threading.Tasks.Task RejectDeliverable_NoPermission_ShouldThrowException()
        {
            // Arrange
            var userId = 1;
            var groupId = 1;
